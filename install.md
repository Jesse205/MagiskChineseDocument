...
如果您已经安装了 Magisk ，**强烈建议**通过 Magisk App 使用“直接安装”方法直接升级。以下教程仅适用于初始安装。
...
如果您的设备在 boot 映像中没有 ramdisk ，则 Magisk 只能劫持 Recovery 分区。对于这些设备，每次启用 Magisk 时，您都必须**启动至 Recovery**。
...
- **(正常通电) → (无 Magisk 系统)**
- **(Recovery 组合键) → (启动屏幕) → (松开所有按钮) → (带有 Magisk 的系统)**
...
::: warning 警告
如果您的三星设备未安装 Android 9.0 或更高版本，则说明这些内容不适用于该设备。
:::

### 安装 Magisk 之前

- 安装 Magisk **将触发 KNOX**
- 首次安装 Magisk **需要**完整的数据擦除（这**不包括在解锁 bootloader 时的数据擦除**）。请在继续之前备份数据。
...
:::
只有当您的设备具有 boot ramdisk 时才能使用第三方 Recovery 进行安装。不再推荐在最新的设备上通过第三方 Recovery 安装 Magisk。如果您遇到任何问题，请使用适当的[修补映像](#修补映像)方法。
